...
WebSocketFrameType handshake::parseHandshake(char* input_frame, int input_len)
{
	// 1. copy char*/len into strings
...
string handshake::trim(string str)
{
	//printf("TRIM\n");
	char* whitespace = " \t\r\n";
...
vector<string> handshake::explode(
	string  theString,
	string  theDelimiter,
...
string handshake::answerHandshake()
{
...
int handshake::makeFrame(WebSocketFrameType frame_type, unsigned char* msg, int msg_length, unsigned char* buffer, int buffer_size)
{
	int pos = 0;
	int size = msg_length;
...
WebSocketFrameType handshake::getFrame(unsigned char* in_buffer, int in_length, unsigned char* out_buffer, int out_size, int* out_length)
{
	//printf("getTextFrame()\n");
